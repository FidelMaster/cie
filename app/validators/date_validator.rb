class DateValidator < ActiveModel::EachValidator
  ...
  def message_limits
    ...
    result = ['la fecha debe ser']
    result.push('mayor o igual a',  computed_options[:on_or_after])  if computed_options[:on_or_after]
    result.push('mayor a',        computed_options[:after])        if computed_options[:after]
    result.push('y')                                           if needs_and
    result.push('menor o igual', computed_options[:on_or_before]) if computed_options[:on_or_before]
    result.push('menor',       computed_options[:before])       if computed_options[:before]
    result.join(' ')
  end
